...
Token DataExtractor::currentToken() const {
    ...
    return Token(MedTokenType::END_OF_FILE, "EOF", -1, -1);
}
...
bool DataExtractor::isAtEnd() const {
    return pos >= tokens.size() || currentToken().type == MedTokenType::END_OF_FILE;
}
...
std::vector<Patient> DataExtractor::getPatients() {
    ...
    while (!isAtEnd()) {
        ...
        if (currentToken().lexeme == "paciente") {
            ...
            if (currentToken().type == MedTokenType::STRING) {
            ...
            while (!isAtEnd() && currentToken().lexeme != "]") {
                if (currentToken().lexeme == "edad") {
                    advance();
                    if (currentToken().lexeme == ":") advance();
                    if (currentToken().type == MedTokenType::INTEGER) {
                    ...
                    advance();
                    if (currentToken().lexeme == ":") advance();
                    if (currentToken().type == MedTokenType::BLOOD_TYPE || currentToken().type == MedTokenType::STRING) {
        ...
        if (currentToken().lexeme == "medico") {
            ...
            while (!isAtEnd() && currentToken().lexeme != "]") {
                if (currentToken().lexeme == "especialidad") {
                    advance();
                    if (currentToken().lexeme == ":") advance();
                    if (currentToken().type == MedTokenType::SPECIALTY) {
                    ...
                    advance();
                    if (currentToken().lexeme == ":") advance();
                    if (currentToken().type == MedTokenType::CODE_ID || currentToken().type == MedTokenType::STRING) {
        ...
        if (currentToken().lexeme == "cita") {
            ...
            while (!isAtEnd() && currentToken().lexeme != "]") {
                if (currentToken().lexeme == "fecha") {
                    advance();
                    if (currentToken().lexeme == ":") advance();
                    if (currentToken().type == MedTokenType::DATE) {
                    ...
                    advance();
                    if (currentToken().lexeme == ":") advance();
                    if (currentToken().type == MedTokenType::TIME) {
...
std::vector<Diagnosis> DataExtractor::getDiagnoses() {
    ...
    while (!isAtEnd()) {
        if (currentToken().type == MedTokenType::END_OF_FILE) break;
        ...
        if (currentToken().lexeme == "diagnostico") {
            ...
            while (!isAtEnd() && currentToken().lexeme != "]") {
                if (currentToken().lexeme == "condicion") {
                    ...
                    advance();
                    if (currentToken().lexeme == ":") advance();
                    if (currentToken().type == MedTokenType::DOSE) {
