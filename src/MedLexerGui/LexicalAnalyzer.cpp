...
std::vector<Token> LexicalAnalyzer::analyze() {
    ...
    } while (token.type != MedTokenType::END_OF_FILE);
...
Token LexicalAnalyzer::nextToken() {
    ...
    if (c == '\0') {
        return Token(MedTokenType::END_OF_FILE, "EOF", startLine, startColumn);
    }
...
Token LexicalAnalyzer::readDelimiter() {
    ...
    advance();
    return Token(MedTokenType::DELIMITER, lexeme, startLine, startColumn);
}
...
Token LexicalAnalyzer::readWord() {
    ...
    if (isSection(lexeme)) return Token(MedTokenType::SECTION, lexeme, startLine, startColumn);
    if (isElement(lexeme)) return Token(MedTokenType::ELEMENT, lexeme, startLine, startColumn);
    if (isAttribute(lexeme)) return Token(MedTokenType::ATTRIBUTE, lexeme, startLine, startColumn);
    if (isSpecialty(lexeme)) return Token(MedTokenType::SPECIALTY, lexeme, startLine, startColumn);
    if (isDose(lexeme)) return Token(MedTokenType::DOSE, lexeme, startLine, startColumn);

    return Token(MedTokenType::UNKNOWN, lexeme, startLine, startColumn);
}
...
Token LexicalAnalyzer::readNumberOrDateOrTime() {
    ...
    if (isValidDate(lexeme)) {
        return Token(MedTokenType::DATE, lexeme, startLine, startColumn);
    }

    if (isValidTime(lexeme)) {
        return Token(MedTokenType::TIME, lexeme, startLine, startColumn);
    }
    ...
    if (onlyDigits) {
        return Token(MedTokenType::INTEGER, lexeme, startLine, startColumn);
    }
...
Token LexicalAnalyzer::readString() {
    ...
    if (currentChar() == '"') {
        ...
        if (isBloodType(innerText)) {
            return Token(MedTokenType::BLOOD_TYPE, lexeme, startLine, startColumn);
        }

        if (isCodeId(innerText)) {
            return Token(MedTokenType::CODE_ID, lexeme, startLine, startColumn);
        }

        return Token(MedTokenType::STRING, lexeme, startLine, startColumn);
    }
